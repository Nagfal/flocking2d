...
def velocity(obser_self = {} , parameters = {}, targetloc=[], v_flock=10.0 ,v_max=0.0):
    ...
    v_desired = [v_desired[0]+v_target[0] , v_desired[1]+v_target[1]]
    n =norm(v_desired)
    v_max_d = [v_desired[0]/ n * v_max , v_desired[1]/ n * v_max ]
    if n<v_max:          
        return v_desired
    else:
        return v_max_d
...
def v_ali(obser_self = {} , parameters = {}):
    ...
    for i in range(len(obser_self['nearby_loc'])):
        ...
        if dist == 0 or v_rela_mag ==0:
            ...
            v_ali = [v_ali[0]+v_rela[0],v_ali[1]+v_rela[1]]
            #vlist.append(v_rela)
...
def v_obstacal(obser_self = {} , parameters = {}):
    ...
    for partner in obser_self['obs_wall']:
        ...
        if dist == 0:
            ...
            dvu= [dist_vec[0] / dist, dist_vec[1] / dist]
            vo = [parameters['p_obstacal'] * (parameters['p_obstacal'] - dist) *dvu[0] , parameters['p_obstacal'] * (parameters['p_obstacal'] - dist) *dvu[1]]
            v_obstacal = [v_obstacal[0]+vo[0] , v_obstacal[1]+vo[1]]
